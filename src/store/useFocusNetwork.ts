...
function sortUsersByNewest<T extends any>(items: T[]): T[] {
  const arr = Array.isArray(items) ? [...items] : [];
...
function mergeUsersByKey(...lists: any[][]): PublicUser[] {
  ...
  return sortUsersByNewest(Array.from(map.values())) as PublicUser[];
}
...
async function fetchUsersSearch(userId: number | null, q: string = ''): Promise<PublicUser[]> {
  ...
  return sortUsersByNewest(all) as PublicUser[];
}
